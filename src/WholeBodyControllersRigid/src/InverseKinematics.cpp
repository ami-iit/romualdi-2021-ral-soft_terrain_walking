...
void WalkingIK::setVerboseMode(bool verboseMode)
{
    m_verbose = verboseMode;
}

bool WalkingIK::initialize(yarp::os::Searchable& ikOption,
                           ...
                           const std::vector<std::string>& jointList)
{
    ...
    m_jointRegularizationWeight
        = ikOption.check("joint_regularization_weight", yarp::os::Value(0.5)).asDouble();
    std::string lFootFrame
        = ikOption.check("left_foot_frame", yarp::os::Value("l_sole")).asString();
    std::string rFootFrame
        = ikOption.check("right_foot_frame", yarp::os::Value("r_sole")).asString();
    ...
    if (m_verbose)
    {
        ...
        yInfo() << "Joint Limits:";
        for (iDynTree::JointIndex jointIdx = 0;
             jointIdx < static_cast<int>(m_ik.reducedModel().getNrOfJoints());
             ++jointIdx)
        {
            ...
            {
                if (!joint->getPosLimits(dof, joint_min, joint_max))
                    continue;
                yInfo() << m_ik.reducedModel().getJointName(jointIdx) << ": "
                        << iDynTree::rad2deg(joint_min) << " to " << iDynTree::rad2deg(joint_max);
            }
        }

        m_baseTransform = m_ik.fullModel()
                              .getFrameTransform(m_ik.fullModel().getFrameIndex(m_lFootFrame))
                              .inverse();

        iDynTree::LinkIndex baseDebug
            = m_ik.reducedModel().getFrameLink(m_ik.reducedModel().getFrameIndex(m_lFootFrame));
        ...
        if (m_additionalFrame.size() != 0)
        {
            yInfo() << "Relative rotation between" << m_lFootFrame << " and " << m_additionalFrame
                    << ":\n"
                    << lchecker.getRelativeTransform(m_lFootFrame, m_additionalFrame)
                           .getRotation()
                           .toString();
        }
    ...
    if (!(jointRegularization.isNull()))
    {
        ...
        if (guessValue->size() > 0
            && static_cast<size_t>(guessValue->size()) != m_ik.reducedModel().getNrOfDOFs())
        {
            yError("[ERROR WALKIK] The jointRegularization list should have the same dimension of "
                   "the number of DoFs of the provided model. Model = %lu, Guess = %lu.",
                   m_ik.reducedModel().getNrOfDOFs(),
                   guessValue->size());
            return false;
        }
...
bool WalkingIK::setModel(const iDynTree::Model& model,
...
bool WalkingIK::updateIntertiaToWorldFrameRotation(const iDynTree::Rotation& inertial_R_worldFrame)
{
    if (m_additionalFrame.size() == 0)
    {
        yError() << "WalkingIK: Cannot update the inertia to left frame rotation if no additional "
                    "frame is provided.";
        return false;
    }
...
bool WalkingIK::setFullModelFeedBack(const iDynTree::VectorDynSize& feedback)
{
    ...
    {
        yError("WalkingIK: The feedback is expected to have the same dimension of the total number "
               "of degrees of freedom of the model. Input -> %d != %lu <- Model.",
               feedback.size(),
               m_ik.fullModel().getNrOfDOFs());
        return false;
    }
    m_feedback = feedback;
    return true;
...
bool WalkingIK::prepareIK()
{
    ...
    iDynTree::LinkIndex base
        = m_ik.fullModel().getFrameLink(m_ik.fullModel().getFrameIndex(m_lFootFrame));

    m_baseTransform
        = m_ik.fullModel().getFrameTransform(m_ik.fullModel().getFrameIndex(m_lFootFrame)).inverse();
    ...
    m_ik.setTargetResolutionMode(m_rFootFrame,
                                 iDynTree::InverseKinematicsTreatTargetAsConstraintFull);

    if (m_additionalFrame.size() != 0)
    {
        ok = m_ik.addRotationTarget(m_additionalFrame,
                                    m_additionalRotation,
                                    m_additionalRotationWeight);
        if (!ok)
        {
        ...
        m_ik.setTargetResolutionMode(m_additionalFrame,
...
bool WalkingIK::setInitialGuess(const iDynTree::VectorDynSize& guess)
{
    ...
    {
        yError("WalkingIK: The guess is expected to have the same dimension of the reduced number "
               "of degrees of freedom. Input -> %d != %lu <- Model.",
               guess.size(),
               m_ik.reducedModel().getNrOfDOFs());
        return false;
    }
...
bool WalkingIK::setDesiredJointConfiguration(
    const iDynTree::VectorDynSize& desiredJointConfiguration)
{
    if (desiredJointConfiguration.size() != m_ik.reducedModel().getNrOfDOFs())
    {
        yError("WalkingIK: The desiredJointConfiguration is expected to have the same dimension of "
               "the reduced number of degrees of freedom. Input -> %d != %lu <- Model.",
...
bool WalkingIK::computeIK(const iDynTree::Transform& leftTransform,
                          ...
                          iDynTree::VectorDynSize& result)
{
    ...
    if (m_additionalFrame.size() != 0)
    {
        m_ik.updateRotationTarget(m_additionalFrame,
                                  leftTransform.getRotation().inverse()
                                      * m_inertial_R_world.inverse() * m_additionalRotation,
                                  m_additionalRotationWeight);
    }
    ...
    ok = m_ik.setDesiredReducedJointConfiguration(m_jointRegularization,
                                                  m_jointRegularizationWeight);
    if (!ok)
    {
    ...
    iDynTree::Position footError
        = desiredRightTransform.getPosition()
          - lchecker.getRelativeTransform(m_lFootFrame, m_rFootFrame).getPosition();
