...
using namespace SoftTerrainWalking::TrajectoryPlanner;
...
bool TrajectoryGenerator::configurePlanner(const yarp::os::Searchable& config)
{
    ...
    if(!WalkingControllers::YarpUtilities::getVectorFromSearchable(config, "referencePosition", m_referencePointDistance))
    {
    ...
    if(!WalkingControllers::YarpUtilities::getVectorFromSearchable(config, "leftZMPDelta", leftZMPDelta))
    {
    ...
    if(!WalkingControllers::YarpUtilities::getVectorFromSearchable(config, "rightZMPDelta", rightZMPDelta))
    {
        yError() << "[configurePlanner] Initialization failed while reading rStancePosition vector.";
        return false;
    }
...
bool TrajectoryGenerator::generateFirstTrajectories(const iDynTree::Position& initialPosition)
{
    ...
    m_desiredPoint(0) = m_referencePointDistance(0) + initialPosition(0) + 1;
    m_desiredPoint(1) = m_referencePointDistance(1) + initialPosition(1);

    iDynTree::Vector2 initialPoint;
    initialPoint(0) = m_referencePointDistance(0) + initialPosition(0);
    initialPoint(1) = m_referencePointDistance(1) + initialPosition(1);

    // add the initial point
    if(!unicyclePlanner->addDesiredTrajectoryPoint(initTime, initialPoint))
    {
                                                    ...
                                                    // const iDynTree::Position &initialCOMPosition)
{
    ...
    // at the beginning iCub has to stop
    m_desiredPoint(0) = m_referencePointDistance(0) + 1;
